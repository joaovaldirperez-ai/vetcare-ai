...
El proyecto requiere Python 3.9 o superior y una cuenta con acceso a la API de OpenAI. Se recomienda usar un entorno virtual para aislar las dependencias.

Comience clonando el repositorio y navegando al directorio del proyecto. Cree un entorno virtual usando Python y active lo. Instale las dependencias del proyecto con pip desde el archivo requirements.txt.

El sistema requiere una clave de API de OpenAI para funcionar. Cree un archivo .env en la raíz del proyecto y agregue su clave en la variable OPENAI_API_KEY. Esta variable se carga automáticamente desde el archivo .env mediante python-dotenv.

Una vez configurado, puede ejecutar la interfaz Streamlit con el comando streamlit run app.py. La aplicación estará disponible en http://localhost:8501 en su navegador.
